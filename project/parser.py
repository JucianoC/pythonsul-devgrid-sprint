...
class Parser:
        ...
        try:
            for group in info_groups:
                ...
                if head == "device":
                    ...
                    result_dict['alarms'] = Parser.decode_alarms(tail)
        except:
            return None
        print(result_dict)
        return result_dict
    ...
    @staticmethod
    def decode_alarms(tail_info):
        """
        Broke and classifique the alarms tail info.
        :param tail_info: tail string generated by the group tail of REGEX_SUBGROUP.
        :return: return a dictionary with the alarm as key and the status as value.
        """
        return_dict = dict()
        data = [value.split('=') for value in tail_info.split(';')]
        for key, value in data:
